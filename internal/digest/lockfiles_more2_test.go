...
func TestLockfileHasher_ComposerLock(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "composer.lock"), []byte(data), 0644); err != nil {
		t.Fatal(err)
	}
...
func TestLockfileHasher_PnpmLock(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "pnpm-lock.yaml"), []byte(yaml), 0644); err != nil {
		t.Fatal(err)
	}
	h := NewLockfileHasher(dir)
	sum, err := h.HashLockfiles()
...
func TestLockfileHasher_GemfileLock(t *testing.T) {
	dir := t.TempDir()
	gem := "GEM\n  specs:\n    rake (13.0.6)\n"
	if err := os.WriteFile(filepath.Join(dir, "Gemfile.lock"), []byte(gem), 0644); err != nil {
		t.Fatal(err)
	}
	h := NewLockfileHasher(dir)
	sum, err := h.HashLockfiles()
...
func TestLockfileHasher_PipfileAndCargo(t *testing.T) {
	...
	cargo := "name = \"rand\"\nversion = \"0.8.5\"\n"
	if err := os.WriteFile(filepath.Join(dir, "Cargo.lock"), []byte(cargo), 0644); err != nil {
		t.Fatal(err)
	}
	h := NewLockfileHasher(dir)
	sum, err := h.HashLockfiles()
